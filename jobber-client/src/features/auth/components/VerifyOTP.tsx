...
const VerifyOTP: FC = (): ReactElement => {
  ...
  const onVerifyOTP = async (): Promise<void> => {
    try {
      ...
      const buyerResponse = await dispatch(buyerApi.endpoints.getCurrentBuyerByUsername.initiate() as unknown as UnknownAction);
      dispatch(addBuyer((buyerResponse.data as IResponse).buyer));
      const sellerResponse = await dispatch(
        sellerApi.endpoints.getSellerByUsername.initiate(`${result.user?.username}`) as unknown as UnknownAction
      );
      dispatch(addSeller((sellerResponse.data as IResponse).seller));
