              ...
              {chatMessages.map((msg: IMessageDocument) => (
                <div
                  ...
                  className={`my-2 flex max-w-[300px] gap-y-6 text-sm ${
                    msg.senderUsername !== seller.username ? 'flex-row-reverse self-end' : ''
                  }`}
                >
                  <img
                    src={seller.profilePicture}
                    className={`h-8 w-8 rounded-full object-cover ${msg.senderUsername !== seller.username ? 'hidden' : ''}`}
                    alt="profile image"
                  />
                  <p
                    className={`ml-2 max-w-[200px] rounded-[10px] bg-[#e4e6eb] px-4 py-2 text-start text-sm font-normal md:max-w-[220px] ${
                      msg.senderUsername !== seller.username ? 'max-w-[200px] rounded-[10px] bg-sky-500 text-white' : ''
                    }`}
                  >
                    {msg.body}
                  </p>
