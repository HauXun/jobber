...
import { parseResumeFromPdf } from 'src/shared/helpers/parse-resume-from-pdf';
...
const AddSeller: FC = (): ReactElement => {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const workExperience = resume.workExperiences.map((e) => {
        return {
          title: e.jobTitle,
          company: e.company,
      ...
      const educations = resume.educations.map((e) => {
        return {
          country: 'Country',
          university: e.school,
          title: 'Title',
      ...
      setExperienceFields(workExperience);

      setEducationFields(educations);
          ...
          <div>
            ...
            <div className="flex flex-row items-center justify-end">
              <input
                ...
                name="file-input"
                id="file-input"
                className="block w-full text-sm text-gray-500 file:me-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:disabled:opacity-50 :disabled:pointer-events-none"
                accept=".pdf"
                onChange={handleFileChange}
              />
            </div>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">PDF.</p>
          </div>
