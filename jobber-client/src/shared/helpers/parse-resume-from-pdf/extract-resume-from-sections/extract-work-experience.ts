import type { FeatureSet, ResumeSectionToLines, TextItem } from '../types';
import type { ResumeWorkExperience } from '../types/types';
import { getBulletPointsFromLines, getDescriptionsLineIdx } from './lib/bullet-points';
import { DATE_FEATURE_SETS, getHasText, hasNumber, isBold } from './lib/common-features';
...
const hasJobTitle = (item: TextItem) => JOB_TITLES.some((jobTitle) => item.text.split(/\s/).some((word) => word === jobTitle));
