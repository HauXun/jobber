...
import { BadRequestError, IAuthDocument, IEmailMessageDetails } from '@hauxun/jobber-shared';
...
export async function resetPassword(req: Request, res: Response): Promise<void> {
  ...
  const hashedPassword: string = await AuthModel.hashPassword(password);
...
export async function changePassword(req: Request, res: Response): Promise<void> {
  ...
  const hashedPassword: string = await AuthModel.hashPassword(newPassword);
  await updatePassword(existingUser.id!, hashedPassword);
  const messageDetails: IEmailMessageDetails = {
    username: existingUser.username,
    template: 'resetPasswordSuccess'
